  ...
  */
   ...
   */
		...
		value: function off(type, func, ctx) {
			...
			if (eventType instanceof Event) eventType = type.type;
   ...
   */
		...
		value: function emit(type) {
			...
			var listenerArgs = Array.from(args).concat([event]);
			eventMap.forEach(function (listeners, listenersEventType) {
				if (
				...
				event.type instanceof RegExp && listenersEventType instanceof RegExp && regexpEqual(listenersEventType, event.type)) {
					listeners.forEach(function (listener) {
						...
						if (listener.once) _this.off(event.type, listener.func, listener.ctx);
					});
				}
			});
   ...
   */
		...
		value: function count(type) {
			...
			if (index(eventType)) {
				...
				var total = 0;
				eventMap.forEach(function (listeners, listenersEventType) {
					if (index(listenersEventType) && useRegExp && eventType.test(listenersEventType)) total += listeners.length;else if (regexpEqual(listenersEventType, eventType)) total += listeners.length;
				});
				return total;
			} else {
				var _total = 0;
...
exports.Event = Event;
//# sourceMappingURL=regexp-events.js.map
